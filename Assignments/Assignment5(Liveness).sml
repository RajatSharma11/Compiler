...
|   unioni (x::xs) i set = let
			   val set = StringSet.union(set,strValue(MAPT.find(i,x))) 	
			   in
			       unioni xs i set
			   end;
...
val y = unioni (IntSet.listItems(intValue(MAPT.find(succ,inst)))) i (strValue(MAPT.find(out,inst)))
...
fun fix_point2  (pred,succ) [] ud (i,out) update  = ((i,out),update)
| fix_point2 (pred,succ) (x::xs) ud (i,out) update  = let
						val io = liveness succ x ud (i,out)
						val change = #3 io
						in
							fix_point2 (pred,succ) (xs) ud (#1 io,#2 io) (update + change)
						end;
								
fun fix_point g inst ud (i,out) update  = if(update=0) then
						(i,out)
				     else
				     		let 
				     			val io = fix_point2 g inst ud (i,out) 0 
				     			val change = #2 io
				     		in
...
| bmGraph (x::xs) (pred,succ) bd bGraph = let
					...
					val key = valOf(MAPT.find(bd,List.nth(x,0)))
					val s1 = createSet successor (IntSet.empty) bd
					val s2 = createSet predecessor (IntSet.empty) bd
					in
						bmGraph xs (pred,succ) bd (MAPT.insert(#1 bGraph,key,s2),MAPT.insert(#2 bGraph,key,s1))
					end;
val gk = #1 cg;
val inst = #3 cg;
val eg = emptyGraph inst (MAPT.empty);
val bbgraph = bmGraph [[1,2],[3],[4]] graph bd (MAPT.empty,MAPT.empty);						
							
val io = fix_point bbgraph inst gk (MAPT.empty,MAPT.empty) 1;
